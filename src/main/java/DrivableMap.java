...
class DrivableMap {
    ...
    public Boolean hasFasterThan(int speed) {
        for (HashMap.Entry<String,Drivable> mapping : drivable_map.entrySet()) {
            if (mapping.getValue().getMaxSpeed() >= speed) {
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable() {
        ...
        for (HashMap.Entry<String,Drivable> mapping : drivable_map.entrySet()) {
            if (mapping.getValue() instanceof Tradable) {
                Tradable item = (Tradable) mapping.getValue();
                lst.add(item);
            }
        }
        return lst;
